...
class Case(Base):
    ...
    # Complainant Details
    # Indexed for fast lookup of cases by complainant (User Dashboard)
    complainant_id = Column(String, ForeignKey("users.id"), nullable=True, index=True) # Optional for now if anonymous
    ...
    # System Metadata
    # Indexed for filtering cases by status (e.g., pending review)
    status = Column(String, default=FIRStatus.DRAFT, index=True) # Enum stored as string
    # Indexed for filtering cases by police station (Police Dashboard)
