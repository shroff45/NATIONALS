...
class Case(Base):
    ...
    # Complainant Details
    complainant_id = Column(String, ForeignKey("users.id"), nullable=True, index=True) # Indexed for "My Cases"
    ...
    # System Metadata
    status = Column(String, default=FIRStatus.DRAFT, index=True) # Indexed for status filtering
    police_station_id = Column(String, nullable=True, index=True) # Indexed for station dashboards
    ...
    # Meta
    created_at = Column(DateTime, default=datetime.utcnow, index=True) # Indexed for chronological sorting
