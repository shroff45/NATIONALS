            ...
            if in_degree >= 10 and out_degree >= 10:
                ...
                try:
                    if nodes_in_cycles is None:
                        # Lazy evaluation: compute globally once O(V+E) instead of per-node
                        # Use strongly connected components instead of simple_cycles O((V+E)*C)
                        nodes_in_cycles = set()
                        for scc in nx.strongly_connected_components(self.graph):
                            if len(scc) > 1:
                                nodes_in_cycles.update(scc)
                        # Include self-loops as valid size-1 cycles
                        nodes_in_cycles.update([u for u, v in nx.selfloop_edges(self.graph)])
