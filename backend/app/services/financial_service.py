...
class FinancialAnalyzer:
    ...
    def _calculate_flow_to_nodes(self, source: str, targets: List[str]) -> float:
        ...
        for target in targets:
            try:
                # OPTIMIZATION: Use itertools.islice for lazy evaluation instead of list(nx.all_simple_paths(...))
                # This prevents exponential memory/time consumption by only materializing up to 5 paths.
                paths = itertools.islice(nx.all_simple_paths(self.graph, source, target, cutoff=5), 5)
                for path in paths:  # Limited to 5 paths
    ...
    def _detect_shell_companies(self):
        ...
        # OPTIMIZATION: Defer simple_cycles computation until actually needed, and cache the result.
        # Computing simple_cycles inside the node loop causes O(V*C) redundant recalculations.
        cycles = None
