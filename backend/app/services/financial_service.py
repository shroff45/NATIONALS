...
class FinancialAnalyzer:
    ...
    def _detect_shell_companies(self):
        ...
        # Pre-compute strongly connected components globally O(V+E)
        # instead of nx.simple_cycles inside the node loop which is O(V * (V+E) * C)
        try:
            # Only components with size > 1 can form a cycle
            scc_list = [comp for comp in nx.strongly_connected_components(self.graph) if len(comp) > 1]
            nodes_in_cycles = set()
