...
class FinancialAnalyzer:
    ...
    def _detect_shell_companies(self):
        ...
        # ⚡ Bolt Optimization: Lazy evaluation of nodes involved in strongly connected components
        # computing simple_cycles inside a per-node loop degrades performance to O(N * (V+E)*C)
        # Using strongly_connected_components is O(V+E) and effectively detects nodes in cycles
        ...
        for node in self.graph.nodes():
            ...
            if in_degree >= 10 and out_degree >= 10:
                ...
                try:
                    # Initialize set once if we find a potential candidate
                    if nodes_in_cycles is None:
                        nodes_in_cycles = set()
                        # Any node in a strongly connected component of size > 1 is in a cycle
                        # Using SCCs is much faster than simple_cycles which enumerates all cycles
