...
class FinancialAnalyzer:
    ...
    def _detect_shell_companies(self):
        ...
        # ⚡ Bolt Optimization: Lazy evaluation of graph cycles
        # nx.simple_cycles is O((V+E)*C) and very expensive.
        # Instead of calculating inside the loop causing O(N * (V+E)*C),
        # compute it once lazily and store nodes in an O(1) lookup set.
        nodes_in_cycles_set = None
        ...
        for node in self.graph.nodes():
            ...
            if in_degree >= 10 and out_degree >= 10:
                ...
                try:
                    # Calculate lazily, only if at least one node triggers the check
                    if nodes_in_cycles_set is None:
                        cycles = list(nx.simple_cycles(self.graph))
                        nodes_in_cycles_set = {n for cycle in cycles for n in cycle}

                    if node in nodes_in_cycles_set:
