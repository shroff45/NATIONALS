...
class FinancialAnalyzer:
    ...
    def _calculate_flow_to_nodes(self, source: str, targets: List[str]) -> float:
        ...
        for target in targets:
            try:
                # Lazily evaluate paths to avoid exponential time/memory on dense graphs
    ...
    def _detect_shell_companies(self):
        ...
        for node in self.graph.nodes():
            ...
            if in_degree >= 10 and out_degree >= 10:
                ...
                try:
                    if cycles is None:
                        # Lazy evaluation and caching to avoid O(V+C) cost if no nodes meet criteria
                        # or repeating computation for multiple nodes
                        cycles = list(nx.simple_cycles(self.graph))
