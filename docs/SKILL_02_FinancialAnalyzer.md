# 💰 SKILL 02: Financial Trail Analyzer (Rakshak)
...
Advanced financial investigation tool that:
- Analyzes transaction patterns
- Detects money laundering (layering, structuring)
- Identifies shell companies
- Maps financial networks
- Generates investigation leads

---

## 🏗️ Backend Implementation

### 1. Schema (`backend/app/schemas/financial.py`)
...
### 2. Service (`backend/app/services/financial_service.py`)
...
from app.services.financial_service import financial_analyzer
from app.core.security import get_current_police_user

router = APIRouter(prefix="/police/financial", tags=["Police - Financial Analyzer"])
...
async def analyze_financial_trail(
    ...
    """
    Analyze financial transaction trail for suspicious patterns
    ...
    """
    try:
        result = financial_analyzer.analyze(request)
        return result
    except Exception as e:
        raise HTTPException(
...
async def test_financial_analysis(
    ...
    return financial_analyzer.analyze(request)
```
...
import { Network, AlertTriangle, TrendingUp, Search } from 'lucide-react';
...
Full implementation with NetworkX graph analysis, 7 anomaly detection algorithms, and interactive visualization.
