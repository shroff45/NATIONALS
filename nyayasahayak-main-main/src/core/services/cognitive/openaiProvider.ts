...
export const chatWithNyayabotOpenAI = async (message: string, ragContext?: string, history: ChatMessage[] = []) => {
    ...
    recentHistory.forEach(msg => {
        if (msg.role === 'user' || msg.role === 'model' || msg.role === 'system' || (msg.role as any) === 'assistant') { // OpenAI uses 'assistant', our app uses 'model' sometimes?
            // App uses 'model' for Gemini, 'assistant' is standard for OpenAI
            const role = msg.role === 'model' ? 'assistant' : (msg.role as any);
