...
export const chatWithNyayabotOpenAI = async (message: string, ragContext?: string, history: ChatMessage[] = []) => {
    ...
    recentHistory.forEach(msg => {
        // @ts-ignore - The ChatMessage role may not exactly match OpenAI but we are handling it
        if (msg.role === 'user' || msg.role === 'assistant' || msg.role === 'model') { // OpenAI uses 'assistant', our app uses 'model' sometimes?
