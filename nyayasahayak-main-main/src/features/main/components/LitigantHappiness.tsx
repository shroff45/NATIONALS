...
const LitigantHappiness: React.FC<LitigantHappinessProps> = ({ t }) => {
    ...
    useEffect(() => {
        if (npsScoreRef.current) {
            gsap.from(npsScoreRef.current, {
                ...
                onUpdate: function () {
                    if (npsScoreRef.current) {
                        // @ts-expect-error GSAP target typing is dynamic
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (npsScore !== null) {
            ...
            try {
                await citizenApi.feedback.submit({
                    nps_score: npsScore,
                    comment: feedback
                });
                setSubmitted(true);
                ...
                // Fail silently or show error - for now allow retry
            } finally {
        ...
        return (
            <AnimatedPageWrapper>
                <div className="w-full h-full flex items-center justify-center">
                    <div className="max-w-4xl mx-auto text-center bg-slate-800/80 backdrop-blur-xl p-8 rounded-2xl shadow-xl border border-emerald-500/20 transition-colors">
        ...
        <AnimatedPageWrapper>
            <div className="max-w-4xl mx-auto flex flex-col justify-center h-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Feedback Form */}
                    <div className="bg-slate-800/60 backdrop-blur-xl border border-slate-700 p-6 rounded-2xl shadow-xl transition-colors hover:border-slate-600">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Array.from({ length: 11 }, (_, i) => i).map(score => (
                                        <button
                                            ...
                                            className={`w-10 h-10 rounded-full font-bold transition-all transform hover:scale-110 text-gray-800 ${npsScore === score
                            ...
                            <button
                                ...
                                className="w-full py-2 px-4 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 disabled:bg-slate-700 disabled:text-slate-500 transition-colors flex justify-center items-center"
                            >
                                {isSubmitting ? (
