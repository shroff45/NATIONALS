...
import { Send, Mic, MicOff, Volume2, Globe, Paperclip, X, FileText, Loader2, Sparkles, RefreshCw, AlertTriangle, Clock } from 'lucide-react';
...
const Nyayabot: React.FC<NyayabotProps> = ({ t, messages, setMessages, currentUser }) => {
    ...
    useEffect(() => {
        const initSqlite = async () => {
            try {
                await sqliteChatService.init();
                const history = await sqliteChatService.getHistory();
                if (history && history.length > 0) {
                    // Map SQLite history to current app format if needed (types match currently)
                    // We prefer history from DB over initial welcome message
                    setMessages(history.map(h => ({
                        role: h.role as any,
                        content: h.content,
                        // timestamp is not part of ChatMessage type in types.ts, so we omit it for state
                    })));
                }
            } catch (error) {
                console.error("Failed to load SQLite history:", error);
            }
        };
        initSqlite();
    }, []);
    ...
    const saveMessageToDb = async (role: 'user' | 'model', content: string) => {
        try {
            await sqliteChatService.saveMessage(role, content);
        } catch (error) {
    ...
    const handleClearChat = async () => {
        if (window.confirm(t('confirm_clear'))) {
            setMessages([{ role: 'model', content: "Hello! I'm NYAYABOT, your friendly guide to the Constitution of India. I'm here to help explain complex topics in a simple way. What would you like to know?" }]);
    ...
    const handleSend = useCallback(async (text: string) => {
        ...
        if (!contentModeration.check(userMessage)) {
            setMessages([...messages, { role: 'user' as const, content: userMessage }, { role: 'system' as const, content: t('moderation_blocked') }]);
            return;
        }
        ...
        try {
            ...
            saveMessageToDb('model', typeof response.text === 'function' ? response.text() : (response.text as unknown as string || ''));

        } catch (error: any) {
            console.error("Error chatting with Nyayabot:", error);
            const errorMessage = error?.message || error?.toString() || t('error_occurred');
            setMessages([...newMessages, { role: 'system' as const, content: `Error: ${errorMessage}` }]);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <AnimatedPageWrapper>
                    ...
                    <div className="px-6 py-4 flex justify-between items-center z-10">
                        ...
                        <div className="flex gap-3">
                            <button onClick={handleClearChat} className="px-4 py-2 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-all font-medium text-sm border border-transparent hover:border-red-100 dark:hover:border-red-900/30" title={t('clear_chat')}>
                                <X className="mr-2 w-4 h-4" /> {t('clear_chat')}
                            </button>
                            <button
                                onClick={() => setIsHistoryModalOpen(true)}
                                className="bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50 text-slate-300 px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2"
                            >
                            ...
                            <button onClick={handleNewChat} className="px-4 py-2 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-gray-700 dark:hover:bg-gray-200 text-sm font-bold transition-all shadow-lg shadow-gray-500/20 transform hover:-translate-y-0.5">
                                <Sparkles className="mr-2 w-4 h-4" /> {t('new_chat')}
                            </button>
                        </div>
                    </div>
                            ...
                            <div key={index} className={`flex items-start gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} group`}>
                                {msg.role === 'model' && (
                                    <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0 shadow-lg transform translate-y-2">
                                        <Sparkles className="text-white text-sm w-4 h-4" />
                                    </div>
                                )}
                                            ...
                                            <div className="space-y-1">
                                                {msg.sources.map((source, i) => (
                                                    <a href={source} target="_blank" rel="noopener noreferrer" key={i} className="flex items-center gap-2 text-xs text-blue-600 dark:text-blue-400 hover:underline truncate">
                                                        <Globe className="text-[10px] opacity-70 w-3 h-3" /> {source}
                                                    </a>
                                                ))}
                                            </div>
                                        </div>
                    ...
                    <div className="p-6 pt-2 bg-gradient-to-t from-gray-50 via-gray-50 to-transparent dark:from-gray-900 dark:via-gray-900 z-10">
                        ...
                        {messages.length <= 1 && !isLoading && (
                            <div className="mb-4 flex flex-wrap gap-2 justify-center">
                                {[
                                    { icon: '⚖️', text: 'What are my rights during arrest?' },
                                    { icon: '📋', text: 'How do I get legal aid?' },
                                    { icon: '🔓', text: 'Am I eligible for bail?' },
                                    { icon: '📄', text: 'Explain Section 304 BNS' },
                                ].map((suggestion, idx) => (
                                    <button
                                        key={idx}
                                        onClick={() => {
                                        ...
                                        className="text-xs bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-3 py-2 rounded-full border border-gray-200 dark:border-gray-700 hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-all flex items-center gap-2 shadow-sm"
                                    >
                            ...
                            <div className="mb-3 p-3 bg-white dark:bg-gray-800 rounded-xl border border-blue-100 dark:border-blue-900/30 shadow-sm flex flex-wrap gap-2">
                                {ragFiles.map(file => (
                                    <div key={file.name} className="flex items-center gap-2 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-3 py-1.5 rounded-lg border border-blue-100 dark:border-blue-800/50">
                                        <FileText className="opacity-70 w-4 h-4" />
                                        <span className="truncate max-w-[150px]">{file.name}</span>
                                        <button onClick={() => removeFile(file.name)} className="ml-1 hover:text-red-500 transition-colors"><X className="w-3 h-3" /></button>
                                    </div>
                                ))}
                        ...
                        <form onSubmit={handleSubmit} className="relative flex items-center gap-2 p-2 bg-white dark:bg-gray-800 rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-black/30 border border-gray-100 dark:border-gray-700">
                            ...
                            <div className="flex items-center gap-1 pr-1">
                                <button type="button" onClick={toggleRecording} className={`p-3 rounded-xl transition-all duration-200 ${isRecording ? 'bg-red-50 text-red-500 animate-pulse' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`} disabled={isLoading} title={isRecording ? "Stop Recording" : "Voice Input"}>
                                    {isRecording ? <MicOff className="text-lg w-5 h-5" /> : <Mic className="text-lg w-5 h-5" />}
                                </button>
            ...
            {/* History Modal */}
            <ChatHistoryModal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} />
        </AnimatedPageWrapper>
    );
};
