...
const JusticeTimeline: React.FC<JusticeTimelineProps> = ({ t, selectedCase, language = 'en' }) => {
    ...
    const formatNum = (n: number | string) => getLocalizedNumber(Number(n) || 0, language);
    ...
    const renderContent = () => {
        ...
        const lastHearingDate = selectedCase.lastHearingDate ? new Date(selectedCase.lastHearingDate) : new Date();
