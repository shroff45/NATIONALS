...
// Extracted CaseItem component wrapped in React.memo to prevent unnecessary re-renders
// ⚡ Bolt: Prevents re-rendering all list items when selectedCase changes
const CaseItem = React.memo(({ c, isSelected, onSelect }: { c: Case, isSelected: boolean, onSelect: (c: Case) => void }) => (
    <div
        onClick={() => onSelect(c)}
...
export const CaseTriage: React.FC<CaseTriageProps> = ({ cases, onSelectCase }) => {
    ...
    const handleSelectCase = useCallback((c: Case) => { setSelectedCase(c); onSelectCase(c); setPrediction(null); }, [onSelectCase]);
