...
const CitizenLayout: React.FC = () => {
    ...
    const navItems = [
        ...
        { path: '/citizen/action', label: 'Legal Action', icon: Scale }, // Merged File & Hub
        { path: '/citizen/predict', label: 'Predictor', icon: TrendingUp },
        ...
        { path: '/citizen/profile', label: 'My Profile', icon: User },
        { path: '/citizen/support', label: 'Help & Support', icon: Phone },
    ];
    ...
    const bottomTabItems = [
        { path: '/citizen/home', label: t('nav_home'), icon: Home },
        { path: '/citizen/action', label: 'Legal', icon: Scale },
                ...
                <div className="max-w-7xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <div className="hidden md:flex items-center gap-4 mr-4">
                            {/* Language Selector - Clean & Simple */}
                            <div className="relative group">
                                <button
                                    onClick={() => setShowLanguageMenu(!showLanguageMenu)}
                                    className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition-all border border-transparent hover:border-slate-700"
                                    title="Choose Language / भाषा चुनें"
                                >
                                    <Globe className="w-4 h-4 text-emerald-400" />
                                    <span>{LANGUAGE_OPTIONS.find(l => l.code === language)?.nativeName || 'English'}</span>
                                    <ChevronDown className="w-3 h-3 opacity-50" />
                                </button>

                                {/* Dropdown menu code remains handled by state, but styling simplified in next chunk if needed */}
                                {showLanguageMenu && (
                                    <>
                                        <div className="fixed inset-0 z-40" onClick={() => setShowLanguageMenu(false)} />
                                        <div className="absolute top-full right-0 mt-2 w-48 bg-slate-900 border border-slate-700 rounded-lg shadow-xl z-50 py-1">
                                            {LANGUAGE_OPTIONS.map((lang) => (
                                                <button
                                                    key={lang.code}
                                                    onClick={() => { setLanguage(lang.code); setShowLanguageMenu(false); }}
                                                    className={`w-full text-left px-4 py-2.5 text-sm hover:bg-slate-800 transition-colors flex items-center justify-between ${language === lang.code ? 'text-emerald-400 bg-emerald-500/5' : 'text-slate-300'}`}
                                                >
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-all ${theme === 'high-contrast' ? 'text-yellow-400 bg-yellow-500/10' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
                            ...
                            {/* Font Size Controls - Standard Buttons */}
                            <div className="flex items-center bg-slate-800/50 rounded-lg p-0.5 border border-slate-700/50">
                                <button onClick={() => setFontSize('small')} className={`px-2 py-1 text-xs font-bold rounded-md transition-all ${fontSize === 'small' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>A-</button>
                                <button onClick={() => setFontSize('normal')} className={`px-2 py-1 text-sm font-bold rounded-md transition-all ${fontSize === 'normal' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>A</button>
                                <button onClick={() => setFontSize('large')} className={`px-2 py-1 text-base font-bold rounded-md transition-all ${fontSize === 'large' ? 'bg-slate-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'}`}>A+</button>
                            </div>
                        </div>
